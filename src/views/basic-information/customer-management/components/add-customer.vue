<template>
  <div>
    <el-dialog v-model="dialogVisible" :title="showTitle" width="520" @close="handleCancel">
      ...
      >
        <el-form-item
          :label="formList1.id !== 0 ? `客户属性: ${formList1.attribute}` : '客户属性'"
        ...
        <el-form-item
          :label="formList1.id !== 0 ? `客户类型: ${formList1.type}` : '客户类型'"
          style="width: 100%; margin-bottom: 24px"
          size="small"
          prop="type"
        >
      ...
      <div v-for="(addresses, index) in formList1.addresses" :key="addresses.id">
        ...
        >
          <div
            ...
            v-if="formList1.addresses.length > 1"
          >
            <el-button
              @click="formList1.addresses.splice(index, 1)"
              link
              type="danger"
              size="small"
            >
          ...
          <div class="Form2" style="display: flex; flex-direction: column; align-items: center">
            <el-form-item
              label="配送地址名称"
              style="width: 440px; margin-bottom: 24px; margin-top: 24px"
              size="small"
              prop="address"
            >
            ...
            <el-form-item
              label="联系人"
              style="width: 440px; margin-bottom: 24px"
              size="small"
              prop="contact"
            >
            ...
            >
              <el-input
                v-model="addresses.detailedAddress"
                style="width: 100%"
                type="textarea"
                :rows="3"
                placeholder="请输入详细地址"
      ...
      <el-row type="flex" justify="center" align="middle">
        <el-button size="small" style="width: 100%; border: 1px dashed #ccc" @click="addForm">
          + 新增
        </el-button>
      </el-row>
...
const rules = reactive({
  attribute: [{ required: true, message: '请选择客户属性', trigger: 'blur' }],
  type: [{ required: true, message: '请选择客户类型', trigger: 'blur' }],
  group: [{ required: true, message: '请选择客户分组', trigger: 'blur' }],
  coding: [{ required: true, message: '请填写客户编码', trigger: 'blur' }],
  organization: [{ required: true, message: '请选择客户所属组织', trigger: 'blur' }],
  name: [{ required: true, message: '请填写客户名称', trigger: 'blur' }],
  abbreviation: [{ required: true, message: '请填写客户简称', trigger: 'blur' }],
  remark: [
    { required: true, message: '请填写备注信息', trigger: 'blur' },
    { max: 200, message: '备注信息不能超过200字', trigger: 'blur' }
  ],
  address: [{ required: true, message: '请填写配送地址名称', trigger: 'blur' }],
